...
class YOLO(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": 'logs/LYC_third_filter_416/best_epoch_weights.h5',
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        font = ImageFont.truetype('font/simhei.ttf', font_size, encoding="utf-8")
        ...
        for i, c in list(enumerate(out_classes)):
            ...
            text = "{} {:.4f}".format(predicted_class, score)
            th = font_size
            # 左右两端预留与线宽相同的填充
            ...
            # 画标签框
            draw.rectangle(
                [(xmin, start_y), (xmin + tw, start_y + th)], fill=self.colors[c])
            # 文字左侧空出线宽大小的填充
            draw.text((xmin + thickness, start_y), text, fill=(0, 0, 0), font=font)
            # 画目标框，线条宽度为thickness
            draw.rectangle(
                [(xmin, ymin), (xmax, ymax)], outline=self.colors[c], width=thickness)

            del draw

        return image, predicted_classes, np.array(out_boxes)
